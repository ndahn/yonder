# Globals
"Globals" is a Yonder-specific term describing nodes not part of the typical playback structures. These nodes are often referenced from many other nodes, e.g. attenuations or busses, and have no playback functions on their own.
...
!!! note
    ...
    The bus a sound will be using is usually specified through the actor mixer it is attached to. When you adjust the game's volume in the settings menu (e.g. master, music, voices, sound effects), this usually modifies a property on these busses. If you notice your custom sword clang is controlled by the voices volume it means you've selected the wrong actor mixer for it.
